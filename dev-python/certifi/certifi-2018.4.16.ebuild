...
python_install_all() {
	...
	local DOC_CONTENTS="
		On Gentoo, we don't use certifi's bundled CA certificates.
...
pkg_postinst() {
	local _show_castore_hint=
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		# This is a new installation
		_show_castore_hint=yes
	else
		local _replacing_version=
		for _replacing_version in ${REPLACING_VERSIONS}; do
			if ! version_is_at_least "2018.4.16" "${_replacing_version}"; then
				_show_castore_hint=yes
				break
			fi
		done
	...
	if [[ -n "${_show_castore_hint}" ]]; then
		elog "Beginning with ${PN}-2018.4.16 we no longer install bundled CA certificates."
		elog "Instead we depend on system's CA store."
	fi
}
